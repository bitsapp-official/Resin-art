            ...
            <div class="flex gap-4 pb-6 border-b border-[#DFD9CE]/40 last:border-b-0">
                ...
                <div class="flex-1 flex flex-col justify-between">
                    ...
                    <div class="flex items-center justify-between mt-3">
                        ...
                        <form method="POST" action="{{ route('cart.update') }}" class="cart-drawer-update-form inline-flex items-center">
                            @csrf
                        ...
                        {{-- Remove Link --}}
                        <form method="POST" action="{{ route('cart.remove') }}" class="cart-drawer-remove-form inline">
                            @csrf
                            <input type="hidden" name="item_id" value="{{ $item->id }}">
                            <button type="submit"
...
@if(isset($cartItems) && $cartItems->count() > 0)
    <div class="border-t border-[#DFD9CE]/60 px-7 py-6 space-y-4 bg-[#FAF8F5]">
        <div class="flex justify-between items-baseline">
            ...
            <span class="font-editorial text-2xl text-[#1C1917] font-light">₹ {{ number_format($cart->total ?? 0) }}</span>
        </div>
