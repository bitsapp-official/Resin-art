<x-app-layout title="Verify Email — Maison Résine">
    <div class="min-h-[75vh] flex items-center justify-center py-10 sm:py-16 px-4 sm:px-6 lg:px-12 xl:px-20 w-full min-w-0">
        <div class="max-w-[1060px] w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">
            ...
            <div class="lg:col-span-5 space-y-3 sm:space-y-5 text-left relative z-10">
                ...
                <h1 class="font-editorial text-3xl sm:text-5xl lg:text-[68px] text-[#1C1917] font-light leading-[1.25] tracking-tight">
                    Verify your <span class="block mt-1 sm:mt-3 italic font-normal text-[#1C1917]">email.</span>
                </h1>
                <p class="text-[13.5px] sm:text-[14px] text-[#78716C] font-light leading-relaxed max-w-sm pt-1">
            ...
            <div class="lg:col-span-7 flex justify-center lg:justify-end relative z-10 w-full min-w-0">
                <div class="w-full max-w-[540px] glass rounded-[1.5rem] sm:rounded-[2.25rem] p-5 sm:p-8 md:p-12 space-y-6 sm:space-y-7 relative overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                    ...
                    <div class="space-y-4 sm:space-y-5 text-[13.5px] sm:text-[14px] text-[#524C46] leading-relaxed font-light">
                        <p>
                        ...
                        <div class="bg-white/70 border border-[#EBE6DD] rounded-xl sm:rounded-2xl p-3.5 sm:p-5 flex items-center space-x-3.5 shadow-xs min-w-0">
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-[9.5px] sm:text-[10px] uppercase tracking-[0.18em] text-[#8E877D] font-semibold">Awaiting Verification For</p>
                                <p class="font-medium text-[#1C1917] text-[13px] sm:text-[13.5px] truncate tracking-normal mt-0.5 break-all">{{ auth()->user()->email }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('verification.send') }}" class="pt-2">
                        @csrf
                        <button type="submit" class="w-full sm:w-auto bg-[#1A1615] hover:bg-[#2C2724] text-white text-[10.5px] sm:text-[11px] uppercase tracking-[0.22em] font-semibold px-6 sm:px-8 py-3.5 sm:py-4 rounded-full transition-all cursor-pointer shadow-sm hover:shadow-md active:scale-[0.99] whitespace-nowrap">
                            Resend Secure Link
                        </button>
                    </form>
