                            ...
                            <div class="absolute top-4 right-4 flex flex-col space-y-2 z-20 opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-300 ease-out">
                                ...
                                <form method="POST" action="{{ route('wishlist.toggle') }}">
                                    ...
                                    <button type="submit" title="{{ $isWishlisted ? 'Remove from Wishlist' : 'Add to Wishlist' }}"
                                            class="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 cursor-pointer shadow-md {{ $isWishlisted ? 'bg-[#1C1917] text-white' : 'glass-pill border border-white/70 text-[#1C1917] hover:bg-[#1C1917] hover:text-white' }}">
